...
dat <- read.table(here::here("processed-data", "09_psychiatric_GSEA","LDSC", "human_sce_aggregated.tsv"),header=T)

dat.norm <- t(apply(dat, 1, function(x){x/sum(x)}))
res <- apply(dat.norm, 2, is_top_10_percent)
rownames(res) <- rownames(dat.norm)
write.csv(res,here::here("processed-data", "09_psychiatric_GSEA","LDSC", "human_sce_score.csv"))