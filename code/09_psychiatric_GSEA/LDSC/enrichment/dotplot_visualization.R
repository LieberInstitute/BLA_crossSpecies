...
###load LDSC results
ldsc_results <- read.csv(file=here::here('code','09_psychiatric_GSEA','LDSC',"enrichment", "ldsc_results.csv"))
...
###plot
pdf(here('plots','09_psychiatric_GSEA','LDSC', "enrichment",'ldsc_results_FDR_0.05.pdf'),width=10,height=10)
...
ggplot(ldsc_results, aes(x = cell, y = trait, size = ifelse(FDR > 0.5, NA, log10fdr), color = Coefficient_z.score)) +
    geom_point() +
    scale_size_continuous(range = c(0, 10)) + # Set minimum size to zero for the size scale
    scale_color_gradient2(low = 'blue', high = 'red', midpoint = 0,
                          name = "Coefficient\n(z-score)") +
